...
class Task1():
    ...
    def next_action(self, state, theta, theta_prime):
        """
    ...
    def controller_task1(self, config_filepath=None, render_mode=False):
        ...
        for e in range(NUM_EPISODES):
            ...
            x, y = state[0], -state[1]
            theta_prime = math.atan((-30-y)/(350-x))
            theta_prime = np.rad2deg(theta_prime)
            if theta_prime < 0:
                theta_prime = abs(theta_prime)
            else:
            ...
            for t in range(TIMESTEPS):
                ...
                theta = state[3]
                action = self.next_action(state, theta, theta_prime)
        ...
        for e in range(NUM_EPISODES):
            ...
            for t in range(TIMESTEPS):
                ...
                if y > -60 and y < 60:
                    ...
                    Flag = True
                    if y < 0:
                        if (x > ran_cen_1x - 80 and x < ran_cen_1x + 80) or (x > ran_cen_3x - 80 and x < ran_cen_3x + 80):
                            theta = state[3]
                            theta_prime = math.atan((y-y)/(350-x))
                            theta_prime = np.rad2deg(theta_prime)
                            if theta_prime < 0:
                                theta_prime = abs(theta_prime)
                            else:
                        ...
                        if (x > ran_cen_2x - 80 and x < ran_cen_2x + 80) or (x > ran_cen_4x - 80 and x < ran_cen_4x + 80):
                            theta = state[3]
                            theta_prime = math.atan((y-y)/(350-x))
                            theta_prime = np.rad2deg(theta_prime)
                            if theta_prime < 0:
